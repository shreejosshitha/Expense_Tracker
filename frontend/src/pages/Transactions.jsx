...
const Transactions = () => {
  ...
  // Filter transactions by selected month and date range
  const filteredTransactions = transactions.filter(txn => {
    ...
    const isInDateRange = (!startDate || new Date(txn.date) >= new Date(startDate)) &&
        ...
        <Grid container spacing={2} sx={{ mt: 3 }}>
          <Grid item xs={6}>
            <TextField
              ...
              sx={{ backgroundColor: 'white', borderRadius: 2, boxShadow: '0px 4px 20px rgba(0, 0, 0, 0.1)' }}
            />
          </Grid>
          <Grid item xs={6}>
            <TextField
